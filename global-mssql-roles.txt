...
import {
  for_each = var.mssql_db_reader_import_id == null ? toset([]) : toset([trimspace(var.mssql_db_reader_import_id)])
  to       = azurerm_role_definition.mssql-db-reader
  id       = each.value
}
...
import {
  for_each = var.mssql_db_restore_import_id == null ? toset([]) : toset([trimspace(var.mssql_db_restore_import_id)])
  to       = azurerm_role_definition.mssql-db-restore
  id       = each.value
}
